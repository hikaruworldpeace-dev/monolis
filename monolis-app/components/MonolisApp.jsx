...
const AVATAR_PALETTE = ["#4F8EF7", "#FF7D9E", "#FFB84D", "#3CCB7F", "#A78BFA", "#38BDF8"];
...
// DBの行（trips + trip_members）を画面が使う trip オブジェクトに変換する
function rowToUiTrip(row, memberRows) {
...
function TripList({ trips, onOpen, onCreate, dark, setDark, onSignOut, userEmail, loading }) {
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-neutral-950 pb-28">
      <div className="px-5 pt-14 pb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onSignOut}
            title={userEmail ? `${userEmail} からログアウト` : "ログアウト"}
            className="w-10 h-10 rounded-full bg-white dark:bg-neutral-900 shadow-sm flex items-center justify-center"
          >
...
function TripCreateForm({ onCreate, onClose }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <button
        disabled={!title || !dest || !start || !end || creating}
        onClick={async () => {
          setCreating(true);
          try {
            await onCreate({ title, destination: dest, startDate: start, endDate: end });
          } finally {
            setCreating(false);
          }
        }}
...
function AuthScreen() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-neutral-950 flex flex-col items-center justify-center px-8">
      <img src={LOGO_FULL} alt="monolis" className="h-16 w-auto mb-2" />
      <p className="text-[13px] text-neutral-400 mb-10">みんなの旅を、もっとスムーズに。</p>

      {sent ? (
        <div className="w-full max-w-xs text-center space-y-3">
          <div className="w-14 h-14 mx-auto rounded-2xl bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center">
            <Mail size={22} style={{ color: ACCENT }} />
          </div>
          <p className="text-[14px] text-neutral-700 dark:text-neutral-200 font-medium">メールを送りました</p>
          <p className="text-[13px] text-neutral-400">
            {email} 宛にログイン用リンクを送信しました。メール内のリンクを開くとログインできます。
          </p>
        </div>
      ) : (
        <div className="w-full max-w-xs space-y-3">
          <input
            ...
            className="w-full rounded-[14px] bg-white dark:bg-neutral-900 border border-transparent focus:border-[#4F8EF7] outline-none px-4 h-12 text-[15px] text-neutral-900 dark:text-white placeholder:text-neutral-400 shadow-sm"
          />
          {error && <p className="text-[12px]" style={{ color: "#EF4444" }}>{error}</p>}
          <button
            disabled={!email || sending}
            onClick={sendMagicLink}
            className="w-full h-12 rounded-[14px] text-white font-medium text-[15px] disabled:opacity-40 transition-opacity flex items-center justify-center gap-2"
            style={{ background: ACCENT }}
          >
...
function JoinTripScreen({ tripId, onJoined, onCancel }) {
  ...
  const join = async () => {
    ...
    const displayName = (user.email || "ゲスト").split("@")[0];
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-neutral-950 flex flex-col items-center justify-center px-8 text-center">
      ...
      <h1 className="text-[18px] font-bold text-neutral-900 dark:text-white mb-2">旅行に招待されました</h1>
      ...
      <button
        disabled={joining}
        onClick={join}
        className="w-full max-w-xs h-12 rounded-[14px] text-white font-medium text-[15px] disabled:opacity-40 flex items-center justify-center gap-2"
        style={{ background: ACCENT }}
      >
...
export default function MonolisApp({ initialTripId = null }) {
  ...
  const [pendingJoinId, setPendingJoinId] = useState(initialTripId);
  ...
  // --- fetch trips the current user belongs to ---
  const fetchTrips = useCallback(async () => {
    ...
    const { data: memberRows } = tripIds.length
      ? await supabase.from("trip_members").select("*").in("trip_id", tripIds)
      : { data: [] };
    const uiTrips = tripRows.map((row) =>
      rowToUiTrip(row, (memberRows || []).filter((m) => m.trip_id === row.id))
    );
    setTrips(uiTrips);
    setLoadingTrips(false);
  }, []);
  ...
  // --- realtime: keep trips in sync across members/devices ---
  useEffect(() => {
    if (!session) return;
    const channel = supabase
      .channel("monolis-trips")
  ...
  // --- if opened via an invite link, decide whether to show the join screen ---
  useEffect(() => {
    if (!session || !pendingJoinId || loadingTrips) return;
    const alreadyMember = trips.some((t) => t.id === pendingJoinId);
    if (alreadyMember) {
      setCurrentId(pendingJoinId);
      setPendingJoinId(null);
    }
  }, [session, pendingJoinId, trips, loadingTrips]);
  ...
  const updateTrip = useCallback(async (updated) => {
    ...
    await supabase.from("trips").update({ data: uiTripToData(updated) }).eq("id", updated.id);
  }, []);
  ...
  if (pendingJoinId && !trips.some((t) => t.id === pendingJoinId) && !loadingTrips) {
    return shell(
      <JoinTripScreen
