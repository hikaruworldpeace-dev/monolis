...
export default function SharedMapTab({ trip, updateTrip, currentMember, onSearchActiveChange }) {
  ...
  const [previewSpot, setPreviewSpot] = useState(null); // 検索直後、まだ候補スポットに入っていない状態
  ...
  // ただし、検索欄にフォーカスしている間（＝スマホのキーボードが開閉するタイミング）は
  // 更新を止める。キーボードの開閉でレイアウトが動くと、候補をタップする瞬間に
  // シートごと位置がズレてタップが外れてしまうため。
  useEffect(() => {
    if (!containerRef.current) return;
    const measure = () => {
      if (searchFocusedRef.current) return;
      setContainerH(containerRef.current.clientHeight || 600);
    };
    measure();
  ...
  // タブを離れる時など、想定外の形でこの画面を離れても
  // 裏側の自動更新が止まったままにならないようにする保険
  useEffect(() => {
  ...
  // ---------- temporary marker for a place that's only in preview (not yet a candidate) ----------
  useEffect(() => {
    if (!mapsReady || !mapObjRef.current) return;
    const google = window.google;
    ...
    if (previewSpot && previewSpot.lat != null && previewSpot.lng != null) {
      previewMarkerRef.current = new google.maps.Marker({
        position: { lat: previewSpot.lat, lng: previewSpot.lng },
        map: mapObjRef.current,
        title: previewSpot.name,
        icon: {
          path: google.maps.SymbolPath.CIRCLE,
          scale: 9,
          fillColor: WARNING,
      ...
      mapObjRef.current.panTo({ lat: previewSpot.lat, lng: previewSpot.lng });
    }
    return () => {
      if (previewMarkerRef.current) {
        previewMarkerRef.current.setMap(null);
  ...
  }, [previewSpot, mapsReady]);

  // ---------- add a spot from a selected Google Place ----------
  // 検索して選んだ場所は、まず「プレビュー」として詳細シートに表示するだけにする。
  // 実際に候補スポットへ追加されるのは「ここ行こう！」を押した時点。
  const addSpotFromPlace = useCallback((place) => {
    let photoUrl = null;
    try {
    ...
    setPreviewSpot(draft);
    setSheetPct(70);
  }, []);

  // プレビュー中の場所に「ここ行こう！」を押した瞬間、初めて候補スポットとして確定する
  const confirmPreviewSpot = () => {
    ...
    const newSpot = {
      ...
      comments: [],
      reactions: {},
      votes: currentMember ? [currentMember.id] : [],
    };
    updateTrip({
      ...trip,
      spots: [...spots, newSpot],
  ...
  const selectedSpot = previewSpot || spots.find((s) => s.id === selectedSpotId);
  const isPreview = !!previewSpot;
    ...
    <div ref={containerRef} className="relative h-full w-full">
          ...
          <div className="flex items-center gap-2 rounded-full bg-neutral-100 dark:bg-neutral-800 px-4 h-11">
            ...
            <input
              ...
              onFocus={() => {
                ...
                onSearchActiveChange?.(true); // 検索中は裏側の自動更新を止める
                setSheetPct((p) => Math.max(p, 60)); // 候補が見やすいようシートを広げる
              }}
              onBlur={() => {
                // タップ操作が終わるまで少し待ってから解除する
                // （候補をタップした瞬間にキーボードが閉じ始めるケースへの対策）
                setTimeout(() => {
      ...
      {selectedSpot && (
        <div className="fixed inset-0 z-40 flex items-end justify-center">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={closeSpotSheet} />
          <div className="relative w-full max-w-md bg-white dark:bg-neutral-900 rounded-t-[20px] max-h-[88vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              <div className="flex items-start justify-between gap-2">
                ...
                <button onClick={closeSpotSheet} className="w-8 h-8 rounded-full bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center shrink-0">
              ...
              {isPreview ? (
                <>
                  <p className="text-[12px] text-neutral-400 leading-relaxed">
                    まだ候補スポットには追加されていません。「ここ行こう！」を押すと、みんなの候補リストに加わります。
                  </p>
                  <button
                    onClick={confirmPreviewSpot}
                    className="w-full h-12 rounded-[14px] font-medium text-[15px] text-white flex items-center justify-center gap-1.5"
                    style={{ background: SUCCESS }}
                  >
                    <ThumbsUp size={16} /> ここ行こう！（候補に追加する）
                  </button>
                </>
              ) : (
                <>
